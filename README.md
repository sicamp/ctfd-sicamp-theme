...
```
git subtree add --prefix CTFd/themes/ctfd-sicamp-theme git@github.com:sicamp/ctfd-sicamp-theme.git main --squash
```

### Pull latest changes to subtree

```
git subtree pull --prefix CTFd/themes/ctfd-sicamp-theme git@github.com:sicamp/ctfd-sicamp-theme.git sicamp --squash
```
...
1. Install [Yarn](https://classic.yarnpkg.com/en/) following the [official installation guides](https://classic.yarnpkg.com/en/docs/install).
...
2. Run `yarn install` in the root of `custom-theme` folder to install the necessary Node packages including `vite`.

3. Run the appropriate yarn build mode:
...
4. Now, you can start your modifications in the `assets` folder. Each time you save, Vite will automatically recompile everything (assuming you are using `yarn dev`), and you can directly see the result by importing your compiled theme into a CTFd instance.
   Note: You do not need the `node_modules` folder, you can simply zip the theme directory without it.

5. When you are ready you can use `yarn build` to build the production copy of your theme.
